...
g = Variable('g',9.8,'m/s/s','Surface gravity on Earth')
rho = Variable('rho',1.225,'kg/m^3','Surface air density on Earth')
mu = Variable("\\mu", 1.73e-5, "kg/m/s", "viscosity of air")
...
h_fuel =  Variable('h_fuel',42.8e6,'J/kg','Fuel heating value')
n_0 = Variable('n_0',0.3,'-','Whole chain propulsion efficiency')

# Masses
m_payload = Variable('m_payload',4,'kg','payload (max requirement)')
m_fuel = Variable('m_fuel','kg','full fuel mass')
rho_fuel = Variable('rho_fuel',840,'kg/m^3','density of fuel (Jet A-1)')
...
N_ult = Variable('N_ult',2,'-','Ultimate load factor')
...
C_L_max = Variable("C_L_max",1.2,'-','Max lift coefficient of wing')
...
W_eng = Variable('W_eng',2.1*0.647,'N','Weight of engine')
...
T = Variable('T','N','Engine thrust') #TJ20

T_max = Variable('T_max',210,'N','Maximum engine thrust')
