...
class Aircraft(Model):
    ...
    def setup(self):
        ...
        self.components = [self.fuselage, self.wing, self.vtail, self.turbofan, self.rear_gear_left, self.rear_gear_right, self.front_gear,self.avionics]

        self.exposed_components = [self.fuselage, self.wing, self.vtail]
        ...
        return self.components, [
            ...
            C_D0 >= sum(p["C_Df"]*p["FF"]*p["Q"]*p["S_wet"] for p in self.exposed_components)/self.wing["S"]
        ]
...
class AircraftP(Model):
    def setup(self, aircraft, state):
        ...
        self.drag_models = [self.wing_aero]
        ...
        return self.perf_models, [
            ...
            self.D >= sum(p["D"] for p in self.drag_models) + 0.5*state["\\rho"]*state["V"]**2*aircraft["C_D0"]*aircraft.wing["S"],
            ...
            self.z_bre >= (state["g"]*self.Range*self.D*aircraft.turbofan["TSFC"])/(state["V"]*aircraft.mass*g),
...
class Wing(Model):
    ...
    def setup(self):
        ...
        b = Variable("b", "m", "span")
        CL_max = Variable("CL_max",1.2,"-","Cl upper limit")
        C_Df = Variable("C_Df",0.004,"-")
        Q = Variable("Q",1,"-")
        FF = Variable("FF",1e-7,"-")
        return [m*g >= S*rho,
                c == (S/A)**0.5,
...
class Fuselage(Model):
    ...
    def setup(self):
        m = Variable("m", 3000,"kg", "mass")
        S_wet = Variable("S_wet", 159.072, "m^2")
        FF = Variable("FF",1+(0.35/(26.27/5.47)),"-")
        C_Df = Variable("C_Df",0.004,"-")
        Q = Variable("Q",1e-7,"-")
...
#         # m = Variable("m",80,"kg","mass")

# class Observable(Model):
#     def setup(self):
#         m = Variable("m",50,"kg","mass")
#         S_wet = Variable("S_wet", 8.175, "m^2")
#         FF = Variable("FF",1+(0.35/(2.734/1.2190)),"-")
#         C_Df = Variable("C_Df",0.004,"-")
#         Q = Variable("Q",1,"-")
...
class Vtail(Model):
    def setup(self):
        m = Variable("m",2000,"kg","mass")
        FF = Variable("FF",1e-7,"-")
        C_Df = Variable("C_Df",0.004,"-")
        Q = Variable("Q",1.03,"-")
        S_wet = Variable("S_wet",58.625,"m^2")
...
class Turbofan(Model):
    def setup(self):
        # Fixed to GEnx-2B67 engine
        # http://adg.stanford.edu/aa241/propulsion/largefan.html

        m = Variable("m",5624.5454,"kg","mass")
        # TSW = Variable("TSW",0.7*26.91e3/522.1,"N/kg")
        T_max = Variable("T_max",260220.96,"N")
        TSFC = Variable("TSFC",0.27,"lb/(lbf*hour)")
        # return [T_max <= TSW*m]
    def dynamic(self,state):
...
class GearPod(Model):
    def setup(self):
        m = Variable("m",5,"kg","mass")
        FF = Variable("FF",1+(0.35/(11.667)),"-")
        C_Df = Variable("C_Df",0.004,"-")
        Q = Variable("Q",1,"-")
        S_wet = Variable("S_wet",3.107,"m^2")

class RearGear(Model):
    def setup(self):
        m = Variable("m",400,"kg","mass")

class FrontGear(Model):
    def setup(self):
        m = Variable("m",73,"kg","mass")

class Avionics(Model):
    def setup(self):
        m = Variable("m",490.7869,"kg","mass")
...
M = Model(MISSION.mtom/MISSION.range, [MISSION, AC])
M.debug()
...
resultsDict = {'GYSCHUVIJMS':float(SOL(AC.wing["b"]).magnitude),'IOKMMJZWEIY':float(SOL(AC.wing["b"]).magnitude),
    'IXDTYZXHXEF':float(SOL(AC.wing["S"]).magnitude),'GURLLRAZYKP':float(SOL(AC.wing["c"]).magnitude)}
