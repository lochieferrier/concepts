...
class AircraftP(Model):
	def setup(self, aircraft, state):
		...
		self.acceleration = Variable('a',1,'m/s/s','acceleration or thrust excess')
		return self.perf_models, [
			aircraft.mass*state['g'] <= 0.5*state['rho']*(state['V']**2)*self.wing_aero["C_L"]*aircraft.wing['S'],
			self.prop_perf['T'] >= self.wing_aero['D'] + self.acceleration*self.aircraft.mass
		]

class Wing(Model):
	def setup(self):
		m = Variable('m','kg', "mass")
		S = Variable("S", 25, "m^2", "surface area")
